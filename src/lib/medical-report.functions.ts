...
const METRICS: Array<{
  ...
  { label: "Steps", column: "steps", unit: "", direction: "neutral", hint: "Daily step count." },
];
...
async function assembleReportData(userId: string, from: string, to: string, sections: Section) {
  ...
  const [profileQ, medsQ, dosesQ, seizQ, bioQ, auraQ] = await Promise.all([
    ...
    supabaseAdmin
      .from("medications")
      .select(
        "id, name, dosage, dosage_amount, dosage_unit, schedule, times_of_day, start_date, end_date, active",
      )
      .eq("user_id", userId),
    supabaseAdmin
    ...
    sections.extras
      ...
      : Promise.resolve({
          data: [] as Array<{ occurred_at: string; led_to_seizure: boolean }>,
          error: null,
        }),
  ]);
  ...
  const dosesByMed = new Map<
    string,
    Array<{ status: string; scheduled_at: string; taken_at: string | null }>
  >();
  for (const d of dosesQ.data ?? []) {
    if (!dosesByMed.has(d.medication_id)) dosesByMed.set(d.medication_id, []);
    dosesByMed
      .get(d.medication_id)!
      .push(d as { status: string; scheduled_at: string; taken_at: string | null });
  }
  const meds = (medsQ.data ?? []).map((m) => {
    ...
    const dosage =
      m.dosage ?? (m.dosage_amount ? `${m.dosage_amount} ${m.dosage_unit ?? ""}`.trim() : null);
    return {
      name: m.name,
      dosage,
...
export const emailMedicalReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabaseAdmin
      .from("medical_reports")
      .select("file_path, window_from, window_to")
      .eq("id", data.reportId)
      .eq("user_id", userId)
      .maybeSingle();
    ...
    const { data: signed, error: sErr } = await supabaseAdmin.storage
      .from("medical-reports")
      .createSignedUrl(row.file_path, 60 * 60 * 24 * 7); // 7 days
    if (sErr) throw new Error(sErr.message);
    ...
    const { data: prof } = await supabaseAdmin
      ...
      .maybeSingle();
    const senderName =
      [prof?.first_name, prof?.last_name].filter(Boolean).join(" ").trim() || "A Purple user";
