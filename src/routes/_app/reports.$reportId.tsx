...
function ReportDetailPage() {
  ...
  if (!report) {
    return (
      <ReportShell title="Clinical report" back={{ to: "/reports/documents", label: "Reports" }}>
        <p className="text-foreground/70">Report not found.</p>
      </ReportShell>
    );
  }
    ...
    <ReportShell title="Clinical report" back={{ to: "/reports/documents", label: "Reports" }}>
      <section className="report-card-strong p-6 sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="rounded-full text-foreground/85 hover:bg-foreground/5 hover:text-foreground"
          >
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="rounded-full text-destructive hover:bg-foreground/5 hover:text-destructive"
          >
      ...
      {report.summary && (
        <section className="mt-6 report-card">
          <p className="report-eyebrow text-foreground/55">Summary</p>
        ...
        <section className="mt-4 grid sm:grid-cols-2 gap-4">
          {report.findings &&
            ...
            (report.findings as string[]).length > 0 && (
              <div className="report-card">
                <p className="report-eyebrow text-foreground/55">Findings</p>
                <ul className="mt-3 text-sm text-foreground/85 space-y-1.5 list-disc pl-4 marker:text-foreground/40">
          ...
          {report.impressions &&
            ...
            (report.impressions as string[]).length > 0 && (
              <div className="report-card">
                <p className="report-eyebrow text-foreground/55">Impressions</p>
                <ul className="mt-3 text-sm text-foreground/85 space-y-1.5 list-disc pl-4 marker:text-foreground/40">
      ...
      <ReportCard className="mt-10">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-foreground text-base font-medium">Medical disclaimer</h3>
      ...
      <MedicalDisclaimer className="mt-4 text-foreground/70 [&_*]:text-foreground/70" />
    </ReportShell>
  );
}
  ...
  return (
    <section className="mt-6 report-card overflow-hidden">
      <div className="flex items-center justify-between gap-3">
        <p className="report-eyebrow text-foreground/55">
      ...
      {expanded && (
        <div className="mt-3 rounded-xl overflow-hidden bg-black/40 border border-foreground/10">
          {isImage ? (
            <img
              src={url}
              alt={title}
    ...
    <section className="mt-4 report-card">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <p className="report-eyebrow text-foreground/55">AI explanation</p>
          {data?.cached && summary?.headline && (
            <span className="text-[10px] text-foreground/40 uppercase tracking-wider">cached</span>
          )}
        </div>
        <Button
      ...
      {summary?.headline && (
        <>
          <p className="mt-3 font-serif text-xl text-foreground leading-snug">{summary.headline}</p>
          {summary.explanation && (
              ...
              {flagged.map((f, i) => (
                ...
                >
                  <p className="text-sm text-foreground">
                    ...
                    {f.value ? <span className="text-foreground/65"> · {f.value}</span> : null}
                  </p>
                  {f.concern && (
                    <p className="mt-1 text-xs text-foreground/65 leading-relaxed">{f.concern}</p>
                  )}
                </li>
              ))}
            </ul>
