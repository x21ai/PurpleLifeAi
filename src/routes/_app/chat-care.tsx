...
function GroupPicker({ onPicked }: { onPicked: (threadId: string) => void }) {
  const [open, setOpen] = React.useState(false);
  const [mode, setMode] = React.useState<"list" | "create">("list");
  const [title, setTitle] = React.useState("Care team");
  const [selected, setSelected] = React.useState<Record<string, boolean>>({});
  const [busy, setBusy] = React.useState(false);
  const qc = useQueryClient();

  const listFn = useServerFn(listGroupThreads);
  const createFn = useServerFn(createGroupThread);
  const fetchMine = useServerFn(listMyCaregivers);

  const groupsQ = useQuery({
    queryKey: ["care-chat", "groups"],
    queryFn: () => listFn(),
    enabled: open,
  });
  ...
  const caregivers = (mineQ.data?.relationships ?? []).filter(
    (r) => r.status === "active" && r.caregiver_id,
  );
  ...
  const handleCreate = async () => {
    const ids = Object.entries(selected)
      ...
      .map(([k]) => k);
    if (ids.length === 0) {
      toast.error("Pick at least one caregiver.");
      return;
    }
    if (!title.trim()) {
    ...
    setBusy(true);
    try {
      const r = await createFn({ data: { title: title.trim(), caregiverIds: ids } });
      await Promise.all([
        qc.invalidateQueries({ queryKey: ["care-chat", "threads"] }),
        qc.invalidateQueries({ queryKey: ["care-chat", "groups"] }),
      ]);
      onPicked(r.threadId);
      setOpen(false);
    } catch (e) {
      toast.error(userMessage(e, "Couldn't create group"));
    } finally {
      setBusy(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          ...
          title="Open or create a group chat"
        >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {mode === "list" ? "Group chats" : "New group"}
          </DialogTitle>
        </DialogHeader>
          ...
          <>
            {groupsQ.isLoading ? (
              ...
              <p className="py-3 text-sm text-muted-foreground">
                No groups yet. Create one to chat with several caregivers at once.
              </p>
            ) : (
                ...
                {groups.map((g) => (
                  <li key={g.id}>
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 px-2 py-3 text-left rounded-lg hover:bg-secondary/40"
                    >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              {mineQ.isLoading ? (
                ...
                <p className="py-2 text-sm text-muted-foreground">
                  No active caregivers yet. Invite someone from Settings → Sharing.
                </p>
              ) : (
                <ul className="max-h-56 overflow-y-auto rounded-lg border border-border divide-y divide-border">
                  {caregivers.map((r) => {
                    const id = r.caregiver_id as string;
                    const checked = !!selected[id];
                    return (
                      <li key={r.id}>
                        <label className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-secondary/40">
                          <Checkbox
                            checked={checked}
                            onCheckedChange={(v) =>
            ...
            <div className="flex gap-2 justify-end pt-1">
              ...
              <Button
                ...
                disabled={busy || caregivers.length === 0}
              >
                {busy ? (
