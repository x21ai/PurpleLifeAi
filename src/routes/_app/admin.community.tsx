    ...
    <div>
      ...
      <ul className="mt-6 space-y-3">
        {reports.map((r) => (
          <li
            key={r.id}
            className={`rounded-2xl border border-border bg-card p-5 ${r.resolved ? "opacity-60" : ""}`}
          >
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>
                {r.post_id ? "Post" : "Comment"} · reporter {r.reporter_id.slice(0, 8)}…
              </span>
