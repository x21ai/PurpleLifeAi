  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const siteOrigin = oauthSiteOrigin(redirectTo);
        ...
        if (provider === "google") {
          ...
          const callback = `${siteOrigin}/oauth/google/callback`;
        ...
        if (provider === "apple") {
          ...
          if (!clientId) {
            return Response.json({ error: "Apple OAuth not configured" }, { status: 500 });
          }
          const callback = `${siteOrigin}/oauth/apple/callback`;
