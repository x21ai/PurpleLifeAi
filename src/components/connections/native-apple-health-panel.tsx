...
export function NativeAppleHealthPanel({ embedded = false }: NativeAppleHealthPanelProps) {
  ...
  const inner = healthKitAuthorized ? (
    <div className="flex flex-col items-center text-center gap-5 py-2">
      <div className="glass-inset flex h-16 w-16 items-center justify-center rounded-2xl text-emerald-600 dark:text-emerald-400">
      ...
      <div className="flex w-full max-w-xs flex-col gap-3">
        <Button size="lg" className="glass-press w-full" onClick={() => void syncNow()} disabled={busy || !loaded}>
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : t("appleHealth.syncNow")}
        </Button>
        <button
          type="button"
          onClick={() => void openSettings()}
          className="glass-press inline-flex items-center justify-center gap-1.5 text-sm font-medium text-primary hover:underline"
        >
    ...
    <div className="flex flex-col items-center text-center gap-5 py-2">
      <div className="glass-inset flex h-16 w-16 items-center justify-center rounded-2xl text-secondary-foreground">
      ...
      <div className="flex w-full max-w-xs flex-col gap-3">
        <Button size="lg" className="glass-press w-full" onClick={() => void connect()} disabled={busy || !loaded}>
      ...
      {permissionDenied && (
        <p className="text-sm text-[color:var(--data-warn)] max-w-sm glass-inset px-4 py-3">
          {t("appleHealth.permissionDenied")}
        </p>
      )}
      {loaded && hasSyncedData && !permissionDenied && (
        <p className="text-xs text-muted-foreground max-w-sm glass-inset px-4 py-3">
  ...
  return (
    <section className="mt-6 sharing-glass-card p-5 sm:p-8">
      {healthKitAuthorized && (
        <span className="glass-pill mb-4 inline-flex items-center gap-1 px-2.5 py-1 text-xs text-emerald-700 dark:text-emerald-300">
