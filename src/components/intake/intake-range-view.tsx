...
export function IntakeRangeView({ range }: { range: "week" | "month" }) {
  ...
  const hRows = (hydration.data ?? []) as Array<{ consumed_at: string; volume_ml: number; kind: string; notes: string | null }>;
  const fRows = (food.data ?? []) as Array<{ consumed_at: string; name: string; calories_kcal: number | null }>;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <SummaryCard icon={<Droplets className="h-4 w-4" />} label="Avg water / day" value={`${avg.water_ml.toLocaleString()} ml`} sub={`${totals.water_ml.toLocaleString()} ml total`} />
        <SummaryCard icon={<UtensilsCrossed className="h-4 w-4" />} label="Avg kcal / day" value={avg.kcal.toLocaleString()} sub={`${totals.kcal.toLocaleString()} total`} />
      </div>
