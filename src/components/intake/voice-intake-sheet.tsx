    ...
    mutationFn: async () => {
      ...
      for (const it of items) {
        ...
        if (recog.kind === "water" && it.volume_ml) {
          await logH({ data: { consumed_at: now, kind: "water", volume_ml: Math.max(1, Math.round(Number(it.volume_ml) || 250)), notes: note || null } });
        } else if (isDrink && it.volume_ml) {
          await logH({ data: { consumed_at: now, kind: "other", volume_ml: Math.max(1, Math.round(Number(it.volume_ml) || 250)), notes: [name, note].filter(Boolean).join(" · ") || null } });
          if (it.calories_kcal) {
            await createFood({ data: { consumed_at: now, name, portion: it.portion || null, calories_kcal: Number(it.calories_kcal) || null, source: "voice", ai_confidence: recog.confidence, note: note || null } });
          }
        } else {
          await createFood({ data: { consumed_at: now, name, portion: it.portion || null, calories_kcal: it.calories_kcal ? Number(it.calories_kcal) : null, source: "voice", ai_confidence: recog.confidence, note: note || null } });
        }
      }
    },
    onSuccess: () => {
        ...
        <div className="mt-5 space-y-4">
          ...
          {stage === "confirm" && recog && (
            <>
                ...
                {items.map((it, idx) => (
                  <div key={idx} className="rounded-2xl ring-1 ring-border p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      {items.length > 1 && (
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setItems((arr) => arr.filter((_, i) => i !== idx))}>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <Input placeholder="Portion" value={it.portion}
                        onChange={(e) => setItems((arr) => arr.map((x, i) => i === idx ? { ...x, portion: e.target.value } : x))} />
                      <Input placeholder="kcal" inputMode="numeric" value={it.calories_kcal}
                        onChange={(e) => setItems((arr) => arr.map((x, i) => i === idx ? { ...x, calories_kcal: e.target.value.replace(/[^0-9]/g, "") } : x))} />
                      <Input placeholder="ml" inputMode="numeric" value={it.volume_ml}
              ...
              <div>
                <Label htmlFor="vnote" className="text-xs">Note (optional)</Label>
                <Textarea id="vnote" value={note} onChange={(e) => setNote(e.target.value)} rows={2} />
              </div>
              <div className="flex items-center justify-end gap-2 pt-2">
                <Button variant="ghost" onClick={reset}><X className="h-4 w-4 mr-1" /> Discard</Button>
                <Button onClick={() => save.mutate()} disabled={save.isPending}>
                  {save.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Sparkles className="h-4 w-4 mr-1.5" /> Save</>}
                </Button>
              </div>
              <p className="text-[11px] text-muted-foreground">AI estimates can be wrong. Edit before saving.</p>
            </>
          )}
        </div>
      </SheetContent>
