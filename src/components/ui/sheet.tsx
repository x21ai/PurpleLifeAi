...
const sheetOverlayClassName =
  "fixed inset-0 z-50 bg-black/45 backdrop-blur-[12px] backdrop-saturate-[1.35] supports-[backdrop-filter]:bg-black/28 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0";

const sheetGlassHandleClassName =
  "mx-auto mb-2 h-1 w-10 shrink-0 rounded-full bg-foreground/12 ring-1 ring-inset ring-foreground/8 shadow-[inset_0_1px_0_rgba(255,255,255,0.22)] backdrop-blur-sm dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]";
...
const sheetVariants = cva(
  "fixed z-50 gap-4 glass-surface p-6 transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        top: "inset-x-0 top-0 rounded-b-2xl data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top",
        bottom:
          "inset-x-0 bottom-0 nav-glass-sheet border-t border-[color:var(--glass-border)] rounded-t-2xl pt-3 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom data-[state=open]:duration-300 data-[state=closed]:duration-200",
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      {side === "bottom" ? <div aria-hidden className={sheetGlassHandleClassName} /> : null}
