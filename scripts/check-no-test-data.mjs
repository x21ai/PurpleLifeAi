...
const IGNORE_DIRS = new Set([
  ...
  ".agents", ".claude", "supabase/seeds", "scripts", "tests", "e2e",
...
const PATTERNS = [
  ...
  { id: "mock-const",   re: /\b(?:const|let|var)\s+(?:mock|fake|dummy|stub|sample|fixture)[A-Z_]/,
                                                                         why: "Variable named mock/fake/dummy/sample/fixture" },
  { id: "mock-array",   re: /\b(?:MOCK|FAKE|DUMMY|SAMPLE|FIXTURE)_[A-Z_]+\s*=/,
                                                                         why: "Uppercase MOCK_/FAKE_/SAMPLE_ constant" },
  { id: "example-email",re: /[\w.+-]+@example\.(?:com|org|net)\b/i,      why: "@example.com address" },
  { id: "test-email",   re: /\b(?:test|fake|dummy)@[\w.-]+\.[a-z]{2,}/i, why: "test@/fake@/dummy@ email" },
  { id: "john-doe",     re: /\bjohn\s+doe\b|\bjane\s+doe\b/i,            why: "John/Jane Doe placeholder" },
  { id: "foo-bar",      re: /["'`](?:foo|bar|baz|qux)\s+(?:foo|bar|baz|qux)["'`]/i,
                                                                         why: "foo/bar/baz placeholder string" },
  { id: "todo-fake",    re: /\/\/\s*(?:TODO|FIXME|HACK)[^\n]*(?:mock|fake|hardcod|placeholder|stub)/i,
                                                                         why: "TODO/FIXME referencing mock/hardcoded data" },
  { id: "hardcoded",    re: /\bhard-?coded?\b/i,                         why: "Comment/string mentions hard-coded" },
  { id: "placeholder-comp", re: /PlaceholderIndex|data-lovable-blank-page-placeholder|REPLACE this/,
                                                                         why: "Lovable placeholder boilerplate" },
  { id: "test-uuid",    re: /\b00000000-0000-0000-0000-0000000000(?:0[1-9]|[1-9]\d)\b/,
                                                                         why: "Sentinel test UUID" },
  { id: "lipsum-url",   re: /loremipsum|placehold(?:er)?\.(?:co|com|it)|via\.placeholder/i,
...
function scanFile(path) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const p of PATTERNS) {
      if (p.re.test(line)) hits.push({ file: rel, line: i + 1, id: p.id, why: p.why, text: line.trim().slice(0, 200) });
    }
  }
  return hits;
}
...
async function scanDatabase() {
  ...
  for (const q of queries) {
    try {
      ...
      if (!r.ok) { console.warn(`[db] ${q.name} -> HTTP ${r.status}`); continue; }
...
if (codeHits.length === 0 && dbHits.length === 0) {
  console.log(`OK: no hard-coded test/placeholder data found in ${SCAN_DIRS.join(", ")}${wantDb ? " or database" : ""}.`);
  process.exit(0);
}
