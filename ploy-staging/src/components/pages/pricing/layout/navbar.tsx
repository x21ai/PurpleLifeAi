/**
 * @ployComponent
 * @ployComponentId pricing-navbar
...
export default function Navbar({ items = links }: { items?: LinkItemProps[] }) {
  return (
    <header className="border-solid border-ploy-neutral-primary-s3 bg-ploy-background-primary/80 sticky z-30 backdrop-blur border-b top-0">
      <div className="h-16 max-w-6xl flex justify-between items-center mx-auto max-md:px-6 md:px-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href="/sign-up"
            className="text-nowrap bg-ploy-background-inverse text-ploy-text-inverse leading-snug font-medium text-xs whitespace-nowrap h-8 flex justify-center items-center gap-2 shadow-sm transition-colors px-3 rounded-full [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 hover:bg-ploy-background-inverse/90"
          >
            {"Get started"}
          </a>
        </div>
