...
| 6 | **www hybrid Worker entry** (`www-entry.ts` + `wrangler.deploy.ploy.jsonc`) | **IN PR** — `ploy-staging/worker/www-entry.ts`, `wrangler.deploy.ploy.jsonc`, `docs/DEPLOY-WWW-PLOY.md`; merge + verify before flip | **No** (deploy) |
...
**Verdict today:** **NO-GO** for www design **deploy**. Hybrid entry infra is in repo (www hybrid PR); flip still blocked on merge order, trunk CI, QA, and owner GO. **GO** for continued staging QA and merging PRs.
...
5. **Merge www hybrid PR** (`www-entry.ts`, `wrangler.deploy.ploy.jsonc`, `build:ploy:www`, `docs/DEPLOY-WWW-PLOY.md`); run `bun run verify:www-ploy-entry`.
6. Extended staging QA → owner sign-off → **then** www flip deploy (`wrangler.deploy.ploy.jsonc` only after explicit GO).
